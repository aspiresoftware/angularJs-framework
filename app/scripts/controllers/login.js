...
LoginCtrl.controller('LoginCtrl', function ($scope, $rootScope,$location, Auth, AuthService, Utility, AUTH_EVENTS, REST_URL, Session) {
  ...
  $scope.authenticate = function(loginDetails){
    ...
    if(!Utility.isUndefinedOrNull(loginDetails)){
      ...
      AuthService.login(REST_URL.AUTHENTICATION).then(function(result){
        ...
        Session.create(result.data.user.token, result.data.user.userName, result.data.user.details.roles[0]);
        $location.url('/home');
      },function(result){
